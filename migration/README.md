# Migration Notes

This directory tracks legacy-system discovery, field mappings, dry-run procedures, and reconciliation notes.

Start with [legacy-inventory.md](./legacy-inventory.md).

Primary working docs:

- [archive-profile.md](./archive-profile.md)
- [legacy-to-new-mapping.md](./legacy-to-new-mapping.md)
- [postgresql-import-flow.md](./postgresql-import-flow.md)
